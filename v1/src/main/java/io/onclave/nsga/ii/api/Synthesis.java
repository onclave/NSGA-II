...
public class Synthesis {
    ...
    public static Population synthesizeChild(Population parent) {
        ...
        while(populace.size() < Configuration.getPOPULATION_SIZE())
            if((Configuration.getPOPULATION_SIZE() - populace.size()) == 1) populace.add(mutation(binaryTournamentSelection(parent)));
            else for(Chromosome chromosome : crossover(binaryTournamentSelection(parent), binaryTournamentSelection(parent))) populace.add(mutation(chromosome));
